...
/** Valley floor along −Z, hills to the sides, gentle rolling snow. */
export function heightAt(x: number, z: number) {
  const rolling = fbm(x * 0.018, z * 0.018, 4) * 7.5;
  const detail = fbm(x * 0.07, z * 0.07, 2) * 1.6;
  const valley = Math.pow(Math.abs(x) / 42, 1.6) * 9;
  return rolling + detail + valley - 3.2;
}
